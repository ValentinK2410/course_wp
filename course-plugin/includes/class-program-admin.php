...
class Program_Admin {
    ...
    public function add_archive_texts_meta_box() {
        ...
        // Показываем только на странице списка программ
        if ('edit.php' !== $pagenow || 'program' !== $post_type) {
            return;
        }
        
        // Получаем сохраненные значения
        $archive_title_main = get_option('program_archive_title_main', __('Программы', 'course-plugin'));
        $archive_title_sub = get_option('program_archive_title_sub', __('обучения и развития', 'course-plugin'));
        $archive_subtitle = get_option('program_archive_subtitle', __('Комплексные программы для достижения профессиональных целей', 'course-plugin'));
        
        ?>
        <div class="notice notice-info" style="margin: 20px 0; padding: 15px;">
            <h2 style="margin-top: 0;"><?php _e('Настройки страницы архива программ', 'course-plugin'); ?></h2>
            <form method="post" action="">
                <?php wp_nonce_field('program_archive_texts', 'program_archive_texts_nonce'); ?>
                <table class="form-table">
                    <tr>
                        ...
                        <td>
                            ...
                            <p class="description"><?php _e('Основная часть заголовка (например: "Программы")', 'course-plugin'); ?></p>
                        </td>
                    </tr>
                    <tr>
                        <th scope="row">
                        ...
                        <td>
                            ...
                            <p class="description"><?php _e('Дополнительная часть заголовка (например: "обучения и развития")', 'course-plugin'); ?></p>
                        </td>
                    </tr>
                    <tr>
                        <th scope="row">
    ...
    /**
     * Сохранение настроек архива программ
     */
    public function save_archive_texts() {
        ...
        if (!isset($_POST['program_archive_texts_nonce']) || !wp_verify_nonce($_POST['program_archive_texts_nonce'], 'program_archive_texts')) {
            return;
        }
        ...
        if (isset($_POST['archive_title_main'])) {
            update_option('program_archive_title_main', sanitize_text_field($_POST['archive_title_main']));
        }
        
        if (isset($_POST['archive_title_sub'])) {
            update_option('program_archive_title_sub', sanitize_text_field($_POST['archive_title_sub']));
        }
        
        if (isset($_POST['archive_subtitle'])) {
            update_option('program_archive_subtitle', sanitize_textarea_field($_POST['archive_subtitle']));
        }
        
        // Показываем сообщение об успехе
        add_action('admin_notices', function() {
            echo '<div class="notice notice-success is-dismissible"><p>' . __('Настройки архива программ успешно сохранены!', 'course-plugin') . '</p></div>';
        });
    }
}
