...
class Course_Moodle_User_Sync {
    ...
    public function sync_user_on_register($user_id) {
        ...
        // Важно: Moodle требует хотя бы один специальный символ (*, -, или #)
        if (empty($plain_password)) {
            // Генерируем пароль с специальными символами для Moodle
            ...
            error_log('Moodle User Sync: ВНИМАНИЕ! Пароль не найден для ' . $user->user_email . ', используется случайный пароль с специальными символами. Пользователь не сможет войти в Moodle с тем же паролем!');
        }
