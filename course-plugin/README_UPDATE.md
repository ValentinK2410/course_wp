# Как обновить course-plugin на сервере

## ⚠️ ВАЖНО: Не используйте `git pull`!

В директории плагина **нет** репозитория Git. Это правильно! Репозиторий находится на GitHub, а плагин - это подкаталог внутри него.
...
- ❌ `git pull` - не работает, т.к. нет репозитория Git
- ❌ `git clone` в директорию плагина - создаст вложенный репозиторий
- ❌ Ручное копирование файлов - можно забыть что-то скопировать
