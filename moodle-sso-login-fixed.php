<?php
/**
 * Moodle SSO Login Handler (ИСПРАВЛЕННАЯ ВЕРСИЯ)
 * 
 * Этот файл должен быть размещен в Moodle по пути:
 * /path/to/moodle/auth/sso/login.php
 * 
 * ВАЖНО: Этот файл должен быть доступен ТОЛЬКО по прямому URL с токеном.
 * Он НЕ должен выполняться автоматически при других запросах.
 */

// Загружаем конфигурацию Moodle
require_once(__DIR__ . '/../../config.php');

// Проверяем, что мы не в админ-панели и не выполняем другие действия
// Это предотвращает выполнение скрипта при просмотре пользователей и других операциях
if (!isset($_GET['token'])) {
    // Если токен не передан, просто завершаем выполнение без редиректа
    // Это предотвращает проблемы при просмотре пользователей
    die();
}
...
if (empty($token)) {
    // Если токен пустой, перенаправляем на страницу входа
    redirect(new moodle_url('/login/index.php'), 'SSO токен не предоставлен', null, \core\output\notification::NOTIFY_ERROR);
}

// Настройки WordPress SSO (можно вынести в конфигурацию Moodle)
...
if (!$user) {
    ...
    redirect(new moodle_url('/login/index.php'), 'Пользователь не найден в системе', null, \core\output\notification::NOTIFY_ERROR);
}
